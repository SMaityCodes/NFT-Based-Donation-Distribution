...
const AdminAdmissionLetters = () => {
  ...
  const fetchAdmissionLetters = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:3001'}/api/admission-letters`, {
        headers: {
  ...
  const handleUpdateStatus = async (status) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:3001'}/api/admission-letters/${selectedLetter.studentId}/status`,
        {
          method: 'PATCH',
          headers: {
