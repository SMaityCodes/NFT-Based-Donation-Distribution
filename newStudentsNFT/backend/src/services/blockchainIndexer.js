...
const startBlockchainIndexer = async () => {
  ...
  contract.on('CampaignDonationReceived', async (campaignId, donorAddress, amount, event) => {
    console.log(`Event: DonationReceived - Campaign: ${campaignId}, Donor: ${donorAddress}, Amount: ${ethers.utils.formatEther(amount)} ETH`);
    try {
      ...
      if (donorDoc) {
        ...
        if (campaignContributionIndex !== -1) {
          ...
          const currentAmount = ethers.utils.parseEther(donorDoc.campaignContributions[campaignContributionIndex].amount);
          donorDoc.campaignContributions[campaignContributionIndex].amount = ethers.utils.formatEther(currentAmount.add(amount));
        } else {
          // Add new campaign contribution
          donorDoc.campaignContributions.push({ campaignId: campaignId.toNumber(), amount: amountEth });
        }
        const currentTotal = ethers.utils.parseEther(donorDoc.totalDonated);
        donorDoc.totalDonated = ethers.utils.formatEther(currentTotal.add(amount));
    ...
    try {
      ...
      await Student.findOneAndUpdate(
        { studentId: studentId.toNumber() },
        {
          ...
          admissionLetterHash: ethers.utils.toUtf8String(studentData.admissionLetterHash), // Convert bytes32 to string
