...
const AdminDashboardMUI = () => {
  ...
  const [approvingStudents, setApprovingStudents] = useState(new Set()); // Track which students are being approved
  ...
  const handleApproveStudent = async (studentAddress) => {
    ...
    // Prevent multiple approvals for the same student
    if (approvingStudents.has(studentAddress)) {
      console.log('Student approval already in progress:', studentAddress);
      return;
    }

    setApprovingStudents(prev => {
      const newApprovingStudents = new Set(prev);
      newApprovingStudents.add(studentAddress);
      return newApprovingStudents;
    });
    try {
      ...
      setApprovingStudents(prev => {
        const newApprovingStudents = new Set(prev);
        newApprovingStudents.delete(studentAddress);
        return newApprovingStudents;
      });
    }
  };
              ...
              {loadingStudents ? (
                ...
                <List>
                  {registeredStudents.filter(s => !s.approved).map((student, index) => (
                    <Paper component={ListItem} key={student.studentAddress || `student-${index}`} elevation={1} sx={{ mb: 2 }}>
                      <ListItemText
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleApproveStudent(student.studentAddress);
                        }}
                        disabled={approvingStudents.has(student.studentAddress)}
                      >
                        {approvingStudents.has(student.studentAddress) ? (
                          <>
              ...
              {loadingApprovedStudents ? (
                ...
                <List>
                  {approvedStudents.map((student, index) => (
                    <Paper component={ListItem} key={student.studentAddress || `approved-${index}`} elevation={1} sx={{ mb: 2 }}>
                      <ListItemText
                        primary={`Address: ${student.studentAddress || 'N/A'}`}
                        secondary={
                          <>
